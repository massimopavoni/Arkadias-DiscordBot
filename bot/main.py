...
@bot.event
async def on_ready():
    # Show Arkai activity, suggesting help command
    await bot.change_presence(status=discord.Status.online,
                              activity=discord.Activity(type=discord.ActivityType.listening,
                                                        name=f'{bot.command_prefix}help'))
    logger.info(f"Arkai is online: command prefix is {bot.command_prefix}")
...
async def shutdown(ctx: discord_commands.Context):
    ...
    await ctx.send(embed=embed_msg)
    # Change Arkai presence to offline
    await bot.change_presence(status=discord.Status.offline)
    logger.info("Arkai is offline")
    # Shutdown bot
    await bot.close()
...
async def main():
    ...
    if ping:
        logger.critical("Arkai won't raise from the sandwiches' crumbles")
